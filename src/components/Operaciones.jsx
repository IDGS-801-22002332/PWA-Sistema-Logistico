...
const NotificationBanner = ({ type, message, onClose }) => {
    ...
    const getStyle = () => {
        switch (type) {
            case 'success':
                return { backgroundColor: '#d1f2eb', borderColor: '#27ae60', color: '#1e8449' };
            case 'error':
                return { backgroundColor: '#fadbd8', borderColor: '#e74c3c', color: '#c0392b' };
            case 'warning':
                return { backgroundColor: '#fef9e7', borderColor: '#f39c12', color: '#d68910' };
            case 'info':
                return { backgroundColor: '#eaf2f8', borderColor: '#3498db', color: '#2980b9' };
            default:
                return {};
        }
    };
...
climatica: { text: "Climática", icon: <Clock size={16} />, color: "var(--info)" },
aduana: { text: "Aduanera", icon: <FileText size={16} />, color: "var(--warning)" },
mecanica: { text: "Mecánica", icon: <Settings size={16} />, color: "var(--danger)" },
documental: { text: "Documental", icon: <FileText size={16} />, color: "var(--secondary)" },
trafico: { text: "Tráfico", icon: <Truck size={16} />, color: "var(--primary)" },
otro: { text: "Otro", icon: <AlertCircle size={16} />, color: "var(--text-secondary)" }
};

// Tipos de incidencia con sus íconos
const TipoIncidenciaDisplay = {
daño_mercancia: { text: "Daño de Mercancía", icon: <AlertTriangle size={16} />, color: "var(--danger)" },
extravio_parcial: { text: "Extravío Parcial", icon: <Package size={16} />, color: "var(--warning)" },
extravio_total: { text: "Extravío Total", icon: <X size={16} />, color: "var(--danger)" },
robo: { text: "Robo", icon: <AlertTriangle size={16} />, color: "var(--danger)" },
error_documentacion: { text: "Error Documentación", icon: <FileText size={16} />, color: "var(--warning)" },
otro: { text: "Otro", icon: <AlertCircle size={16} />, color: "var(--text-secondary)" }
};

// Estados de incidencia con sus íconos
const EstatusIncidenciaDisplay = {
reportada: { text: "Reportada", icon: <AlertTriangle size={16} />, color: "var(--warning)" },
en_revision: { text: "En Revisión", icon: <Clock size={16} />, color: "var(--info)" },
resuelta: { text: "Resuelta", icon: <CheckCircle size={16} />, color: "var(--success)" },
escalada: { text: "Escalada", icon: <AlertCircle size={16} />, color: "var(--danger)" }
};
...
const operacionDemoras = demoras.filter(d => d.id_operacion === operacion.id_operacion);
...
const data = {
...demoraForm,
costo_asociado: demoraForm.costo_asociado ? parseFloat(demoraForm.costo_asociado) : 0
};
createDemora(data);
setDemoraForm({
id_operacion: operacion.id_operacion,
...
fecha_hora_demora: new Date(demora.fecha_hora_demora).toISOString().slice(0, 16),
...
<h5 style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--warning)', margin: 0, borderBottom: '2px solid var(--warning)', paddingBottom: '0.5rem' }}>
⏰ Demoras
</h5>
<button 
...
<form onSubmit={editingDemora ? handleUpdateDemora : handleCreateDemora} style={{ 
marginBottom: '1rem', 
padding: '1rem', 
...
required
>
{Object.entries(TipoDemoraDisplay).map(([key, value]) => (
...
<Save size={14} /> {editingDemora ? 'Actualizar' : 'Guardar'} Demora
</button>
<button type="button" onClick={cancelEdit} className="btn btn-secondary" style={{ padding: '0.4rem 0.8rem' }}>
<X size={14} /> Cancelar
</button>
</div>
</form>
...
const tipoInfo = TipoDemoraDisplay[demora.tipo_demora] || TipoDemoraDisplay.otro;
return (
<div key={demora.id_demora} style={{ 
padding: '1rem', 
backgroundColor: 'var(--light-bg)', 
borderRadius: '8px', 
...
<div style={{ display: 'flex', gap: '0.5rem' }}>
<button 
onClick={() => handleEditDemora(demora)}
style={{ 
background: 'none', 
...
<button 
onClick={() => deleteDemora(demora.id_demora)}
style={{ 
background: 'none', 
...
{new Date(demora.fecha_hora_demora).toLocaleString()}
</div>
...
<DollarSign size={14} style={{ verticalAlign: 'middle', marginRight: '0.25rem' }} />
Costo: {demora.moneda || 'USD'} {demora.costo_asociado}
</div>
)}
</div>
);
...
const operacionIncidencias = incidencias.filter(i => i.id_operacion === operacion.id_operacion);
...
fecha_hora_incidencia: new Date(incidencia.fecha_hora_incidencia).toISOString().slice(0, 16),
descripcion_incidencia: incidencia.descripcion_incidencia || '',
tipo_incidencia: incidencia.tipo_incidencia,
estatus: incidencia.estatus,
fecha_resolucion: incidencia.fecha_resolucion ? new Date(incidencia.fecha_resolucion).toISOString().slice(0, 16) : '',
...
<div style={{ marginBottom: '2rem' }}>
<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
<h5 style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--danger)', margin: 0, borderBottom: '2px solid var(--danger)', paddingBottom: '0.5rem' }}>
⚠️ Incidencias
</h5>
<button 
...
<form onSubmit={editingIncidencia ? handleUpdateIncidencia : handleCreateIncidencia} style={{ 
marginBottom: '1rem', 
padding: '1rem', 
backgroundColor: 'var(--light-bg)', 
borderRadius: '8px', 
...
required
>
{Object.entries(TipoIncidenciaDisplay).map(([key, value]) => (
<option key={key} value={key}>{value.text}</option>
))}
</select>
</div>
...
className="form-input"
>
{Object.entries(EstatusIncidenciaDisplay).map(([key, value]) => (
<option key={key} value={key}>{value.text}</option>
))}
</select>
</div>
...
<div style={{ marginBottom: '1rem' }}>
<label className="form-label">Comentarios de Resolución</label>
<textarea
...
<div style={{ display: 'flex', gap: '0.5rem' }}>
<button type="submit" className="btn btn-danger" style={{ padding: '0.4rem 0.8rem' }}>
<Save size={14} /> {editingIncidencia ? 'Actualizar' : 'Reportar'} Incidencia
</button>
<button type="button" onClick={cancelEditIncidencia} className="btn btn-secondary" style={{ padding: '0.4rem 0.8rem' }}>
<X size={14} /> Cancelar
</button>
</div>
</form>
...
const tipoInfo = TipoIncidenciaDisplay[incidencia.tipo_incidencia] || TipoIncidenciaDisplay.otro;
const estatusInfo = EstatusIncidenciaDisplay[incidencia.estatus] || EstatusIncidenciaDisplay.reportada;
return (
<div key={incidencia.id_incidencia} style={{ 
padding: '1rem', 
backgroundColor: 'var(--light-bg)', 
borderRadius: '8px', 
border: `1px solid ${tipoInfo.color}`,
...
<span style={{ 
padding: '0.25rem 0.5rem', 
backgroundColor: estatusInfo.color, 
color: 'white', 
borderRadius: '12px', 
fontSize: '0.75rem', 
fontWeight: 600 
...
<button 
onClick={() => deleteIncidencia(incidencia.id_incidencia)}
style={{ 
background: 'none', 
border: '1px solid var(--danger)', 
color: 'var(--danger)', 
...
<div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>
<Clock size={14} style={{ verticalAlign: 'middle', marginRight: '0.25rem' }} />
Reportada: {new Date(incidencia.fecha_hora_incidencia).toLocaleString()}
{incidencia.fecha_resolucion && (
<span style={{ marginLeft: '1rem' }}>
<CheckCircle size={14} style={{ verticalAlign: 'middle', marginRight: '0.25rem', color: 'var(--success)' }} />
Resuelta: {new Date(incidencia.fecha_resolucion).toLocaleString()}
</span>
)}
</div>
<p style={{ margin: '0.5rem 0', fontSize: '0.9rem' }}>
...
<strong style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Resolución:</strong>
...
<label className="form-label" htmlFor="numero_referencia_proveedor">
Referencia / Booking*
</label>
<input
...
<option key={cot.id_cotizacion || cot.id} value={cot.id_cotizacion || cot.id}>
#{cot.id_cotizacion || cot.id} - {cot.cliente_nombre || cot.nombre_cliente || 'Sin cliente'} - {cot.tipo_servicio}
</option>
))}
</select>
) : (
...
<div className="form-group">
<label className="form-label" htmlFor="id_usuario_operativo">
Encargado Operativo*
</label>
{usuarios.length > 0 ? (
<select
...
{usuarios.filter(usuario => usuario.rol === 'operaciones').map(usuario => (
...
{proveedor.nombre_empresa || proveedor.nombre || `Proveedor ${proveedor.id_proveedor || proveedor.id}`} - {proveedor.tipo_servicio_ofrecido || 'N/A'}
</option>
))}
</select>
) : (
...
{agente.nombre || 'Agente'} {agente.apellido || ''} - {agente.tipo_agente || 'General'}
</option>
))}
</select>
) : (
...
<div className="form-group">
<label className="form-label" htmlFor="fecha_estimada_arribo">
...
<div className="form-group">
<label className="form-label" htmlFor="fecha_estimada_entrega">
...
<div className="form-group" style={{ gridColumn: "1 / -1" }}>
<label className="form-label">Descripción de Mercancía (Referencia)</label>
<div style={{ 
padding: '0.75rem', 
...
>
<div style={{ padding: '1.5rem', borderRight: isDetailOpen ? 'none' : '1px solid #e5e7eb' }}>
{/* Header */}
...
<label className="form-label" style={{ fontSize: '0.85rem' }}>Cliente {operacion.id_cotizacion && `(Cot. #${operacion.id_cotizacion})`}</label>
...
<small style={{ color: 'var(--text-secondary)' }}>ID Cliente: {operacion.id_cliente}</small>
</div>
...
<small style={{ color: 'var(--text-secondary)' }}>ID Usuario: {operacion.id_usuario_operativo}</small>
</div>

<div className="form-group" style={{ margin: 0 }}>
<label className="form-label" style={{ fontSize: '0.85rem' }}>Proveedor</label>
<p style={{ margin: 0, fontWeight: 600, fontSize: '1rem' }}>
<Building2 size={16} style={{ verticalAlign: 'middle', marginRight: '8px', color: 'var(--info)' }} />
{operacion.proveedor_nombre}
</p>
<small style={{ color: 'var(--text-secondary)' }}>ID Proveedor: {operacion.id_proveedor}</small>
</div>
...
<p style={{ margin: 0, fontWeight: 600, fontSize: '1rem' }}>
<User size={16} style={{ verticalAlign: 'middle', marginRight: '8px', color: 'var(--warning)' }} />
{operacion.agente_nombre}
</p>
<small style={{ color: 'var(--text-secondary)' }}>ID Agente: {operacion.id_agente}</small>
</div>
)}

{operacion.id_cotizacion && (
<div className="form-group" style={{ margin: 0 }}>
<label className="form-label" style={{ fontSize: '0.85rem' }}>Cotización Origen</label>
<p style={{ margin: 0, fontWeight: 600, fontSize: '1rem', color: 'var(--info)' }}>
<Settings size={16} style={{ verticalAlign: 'middle', marginRight: '8px' }} />
Cotización #{operacion.id_cotizacion}
</p>
</div>
)}
</div>
...
<p style={{ margin: 0, fontWeight: 600, fontSize: '1rem' }}>
{getServiceIcon(operacion.tipo_servicio)}
<span style={{ marginLeft: '8px' }}>{TipoServicioDisplay[operacion.tipo_servicio]}</span>
</p>
</div>
...
<p style={{ margin: 0, fontWeight: 600, fontSize: '1rem' }}>
<Package size={16} style={{ verticalAlign: 'middle', marginRight: '8px', color: 'var(--secondary)' }} />
{operacion.tipo_carga}
</p>
</div>
...
<div className="form-group" style={{ margin: 0 }}>
<label className="form-label" style={{ fontSize: '0.85rem' }}>Booking/Referencia</label>
<p style={{ margin: 0, fontWeight: 600, fontSize: '1rem' }}>
<Settings size={16} style={{ verticalAlign: 'middle', marginRight: '8px', color: 'var(--primary)' }} />
...
{/* Fechas */}
{(operacion.fecha_estimada_arribo || operacion.fecha_estimada_entrega) && (
<div style={{ marginBottom: '2rem' }}>
...
<label className="form-label" style={{ fontSize: '0.85rem', color: 'var(--primary)' }}>Fecha de Arribo</label>
<p style={{ margin: 0, fontWeight: 600, fontSize: '1.1rem' }}>
<Calendar size={18} style={{ verticalAlign: 'middle', marginRight: '8px', color: 'var(--primary)' }} />
{new Date(operacion.fecha_estimada_arribo).toLocaleDateString()}
</p>
</div>
)}
...
<div className="form-group" style={{ margin: 0, textAlign: 'center', padding: '1rem', backgroundColor: 'var(--light-bg)', borderRadius: '8px' }}>
<label className="form-label" style={{ fontSize: '0.85rem', color: 'var(--danger)' }}>Fecha de Entrega</label>
<p style={{ margin: 0, fontWeight: 600, fontSize: '1.1rem' }}>
<Clock size={18} style={{ verticalAlign: 'middle', marginRight: '8px', color: 'var(--danger)' }} />
{new Date(operacion.fecha_estimada_entrega).toLocaleDateString()}
</p>
</div>
)}
</div>
...
<div style={{ marginBottom: '2rem' }}>
<h5 style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--secondary)', marginBottom: '1rem', borderBottom: '2px solid var(--secondary)', paddingBottom: '0.5rem' }}>
📝 Notas de la Operación
</h5>
<div style={{ 
padding: '1rem', 
...
<TrackingSection 
operacion={operacion}
tracking={tracking.filter(t => t.id_operacion === operacion.id_operacion)}
...
<button className="btn btn-secondary" onClick={() => onAction("edit", operacion)}>
<Settings size={16} /> Editar Operación
</button>
<button className="btn btn-danger" onClick={() => onAction("delete", operacion)}>
...
<label className="form-label" style={{ fontSize: "0.8rem" }}>Fechas Estimadas</label>
{operacion.fecha_estimada_arribo && (
<div style={{ fontSize: "0.9rem", color: "var(--primary)", fontWeight: 600, marginTop: "0.3rem" }}>
<Calendar size={14} style={{ verticalAlign: "middle", marginRight: 6 }} />
Arribo: {new Date(operacion.fecha_estimada_arribo).toLocaleDateString()}
</div>
)}
{operacion.fecha_estimada_entrega && (
<div style={{ fontSize: "0.9rem", color: "var(--danger)", fontWeight: 600, marginTop: "0.3rem" }}>
<Clock size={14} style={{ verticalAlign: "middle", marginRight: 6 }} />
Entrega: {new Date(operacion.fecha_estimada_entrega).toLocaleDateString()}
</div>
)}
</div>
...
const TrackingSection = ({ 
    ...
    const mapLat = latestTracking?.latitud ? parseFloat(latestTracking.latitud) : defaultLat;
    const mapLng = latestTracking?.longitud ? parseFloat(latestTracking.longitud) : defaultLng;
    ...
    const handleSubmit = async () => {
        ...
        const payload = {
            ...
            latitud: newTracking.latitud && newTracking.latitud !== '' ? parseFloat(newTracking.latitud) : null,
            longitud: newTracking.longitud && newTracking.longitud !== '' ? parseFloat(newTracking.longitud) : null,
            estatus_seguimiento: newTracking.estatus_seguimiento,
            referencia_transportista: newTracking.referencia_transportista?.trim() || null,
            nombre_transportista: newTracking.nombre_transportista?.trim() || null,
    ...
    const handleEdit = (trackingItem) => {
        setNewTracking({
            ...
            referencia_transportista: trackingItem.referencia_transportista || '',
    ...
    const getStatusColor = (status) => {
        switch (status) {
            case 'en_origen': return '#6c757d';
            case 'en_transito': return '#007bff';
            case 'en_destino': return '#ffc107';
            case 'entregado': return '#28a745';
            case 'aduana': return '#dc3545';
    ...
    return (
        <div style={{ marginBottom: '1.5rem' }}>
            <div style={{ 
                ...
                >
                    <Plus size={16} />
                    {editingTracking ? 'Cancelar Edición' : 'Actualizar Ubicación'}
                </button>
            </div>
                        ...
                        <Popup>
                            <div style={{ textAlign: 'center' }}>
                                <h5 style={{ margin: '0 0 0.5rem 0' }}>
                                    {latestTracking ? latestTracking.ubicacion_actual || 'Ubicación Actual' : 'Ubicación por Defecto'}
                                </h5>
                                {latestTracking && (
                                    <>
                                        <p style={{ margin: '0.25rem 0', fontSize: '0.85rem' }}>
                                            <strong>Estado:</strong> {getStatusText(latestTracking.estatus_seguimiento)}
                                        </p>
                                        <p style={{ margin: '0.25rem 0', fontSize: '0.85rem' }}>
                                            <strong>Actualizado:</strong> {new Date(latestTracking.fecha_hora_actualizacion).toLocaleString()}
                                        </p>
                                    </>
                                )}
                            </div>
            ...
            {isAddingTracking && (
                <div className="tracking-form">
                    <h5>
                        {editingTracking ? 'Editar Ubicación' : 'Nueva Actualización de Tracking'}
                    </h5>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setNewTracking({...newTracking, ubicacion_actual: e.target.value})}
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={(e) => setNewTracking({...newTracking, estatus_seguimiento: e.target.value})}
                                style={{
                                    width: '100%',
                                    padding: '0.5rem',
                                    border: '1px solid #ddd',
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setNewTracking({...newTracking, latitud: e.target.value})}
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setNewTracking({...newTracking, longitud: e.target.value})}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                        <div>
                            ...
                            <select
                                ...
                                onChange={(e) => setNewTracking({...newTracking, nombre_transportista: e.target.value})}
                                style={{
                                    width: '100%',
                                    padding: '0.5rem',
                                    border: '1px solid #ddd',
                            ...
                            >
                                ...
                                {proveedores.map(proveedor => (
                                    <option 
                                        ...
                                        value={proveedor.nombre_empresa || proveedor.nombre || `Proveedor ${proveedor.id_proveedor || proveedor.id}`}
                                    >
                                        {proveedor.nombre_empresa || proveedor.nombre || `Proveedor ${proveedor.id_proveedor || proveedor.id}`}
                                        {proveedor.tipo_servicio_ofrecido && ` - ${proveedor.tipo_servicio_ofrecido}`}
                                    </option>
                                ))}
                            </select>
                        </div>
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setNewTracking({...newTracking, referencia_transportista: e.target.value})}
                    ...
                    <div style={{ marginBottom: '1rem' }}>
                        ...
                        <textarea
                            ...
                            onChange={(e) => setNewTracking({...newTracking, notas_tracking: e.target.value})}
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                        ...
                        >
                            <X size={16} style={{ verticalAlign: 'middle', marginRight: '0.25rem' }} />
                            Cancelar
                        </button>
                        <button
                        ...
                        >
                            <Save size={16} style={{ verticalAlign: 'middle', marginRight: '0.25rem' }} />
            ...
            <div>
                ...
                {tracking.length === 0 ? (
                    <div className="tracking-empty-state">
                        <Map size={48} style={{ color: 'var(--text-secondary)', marginBottom: '1rem' }} />
                        <p style={{ margin: 0, fontSize: '0.9rem' }}>
                            No hay actualizaciones de tracking para esta operación.
                            <br />
                            Agrega la primera ubicación para comenzar el seguimiento.
                        </p>
                    </div>
                ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        {tracking.map((trackingItem, index) => (
                            <div 
                                ...
                                className={`tracking-history-item ${index === 0 ? 'tracking-current-item' : ''}`}
                                style={{
                                    borderLeft: `4px solid ${getStatusColor(trackingItem.estatus_seguimiento)}`
                                }}
                            >
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem' }}>
                                    <div style={{ flex: 1 }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
                                            <span style={{
                                                padding: '0.25rem 0.5rem',
                                                backgroundColor: getStatusColor(trackingItem.estatus_seguimiento),
                                                color: 'white',
                                                borderRadius: '12px',
                                                fontSize: '0.75rem',
                                                fontWeight: '500'
                                            }}>
                                                {getStatusText(trackingItem.estatus_seguimiento)}
                                            </span>
                                            {index === 0 && (
                                                <span style={{
                                                    padding: '0.25rem 0.5rem',
                                        ...
                                        <h6 style={{ margin: '0 0 0.5rem 0', fontSize: '1rem', color: 'var(--text-primary)' }}>
                                            {trackingItem.ubicacion_actual || 'Ubicación no especificada'}
                                        </h6>
                                        
                                        <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>
                                            <Clock size={14} style={{ verticalAlign: 'middle', marginRight: '0.25rem' }} />
                                            Actualizado: {new Date(trackingItem.fecha_hora_actualizacion).toLocaleString()}
                                        </div>
                                        
                                        {(trackingItem.latitud && trackingItem.longitud) && (
                                            <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>
                                                <Crosshair size={14} style={{ verticalAlign: 'middle', marginRight: '0.25rem' }} />
                                                Coordenadas: {parseFloat(trackingItem.latitud).toFixed(6)}, {parseFloat(trackingItem.longitud).toFixed(6)}
                                            </div>
                                        )}
                                        
                                        {(trackingItem.nombre_transportista || trackingItem.referencia_transportista) && (
                                            <div style={{ 
                                                fontSize: '0.85rem', 
                                                color: 'var(--text-secondary)', 
                                                marginBottom: '0.5rem',
                                                ...
                                                {trackingItem.nombre_transportista && (
                                                    <span>
                                                        <Truck size={14} style={{ verticalAlign: 'middle', marginRight: '0.25rem' }} />
                                                        {trackingItem.nombre_transportista}
                                                    </span>
                                                )}
                                                {trackingItem.referencia_transportista && (
                                                    <span>
                                                        ...
                                                        Ref: {trackingItem.referencia_transportista}
                                                    </span>
                                                )}
                                            </div>
                                        )}
                                        ...
                                        {trackingItem.notas_tracking && (
                                            <div className="tracking-notes">
                                                <strong style={{ color: 'var(--text-secondary)' }}>Notas:</strong>
                                    ...
                                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                                        <button 
                                            onClick={() => handleEdit(trackingItem)}
                                            style={{ 
                                                background: 'none', 
                                                border: '1px solid var(--primary)', 
                                                color: 'var(--primary)', 
                                        ...
                                        <button 
                                            onClick={() => deleteTracking(trackingItem.id_tracking)}
                                            style={{ 
                                                background: 'none', 
                                                border: '1px solid var(--danger)', 
                                                color: 'var(--danger)', 
...
useEffect(() => {
    ...
    async function loadData() {
        try {
            ...
            const [opsData, clientesData, usuariosData, proveedoresData, agentesData, cotizacionesData, localizacionesData, demorasData, incidenciasData, trackingData] = await Promise.allSettled([
                ...
                fetch("https://pwa-sistema-logistico-backend.onrender.com/demoras").then(res => res.ok ? res.json() : []).catch(() => []),
                fetch("https://pwa-sistema-logistico-backend.onrender.com/incidencias").then(res => res.ok ? res.json() : []).catch(() => []),
                fetch("https://pwa-sistema-logistico-backend.onrender.com/tracking").then(res => res.ok ? res.json() : []).catch(() => [])
            ]);
            ...
            if (mounted) {
                ...
                if (trackingData.status === 'fulfilled' && Array.isArray(trackingData.value)) {
                    setTracking(trackingData.value);
                }

                // Procesar operaciones
                if (opsData.status === 'fulfilled' && Array.isArray(opsData.value)) {
                    const enrichedOps = (opsData.value || []).map((op) => ({
                        ...op,
                        cliente_nombre: op.cliente?.nombre_empresa || "N/A",
                        usuario_operativo_nombre: `${op.usuario_operativo?.nombre || "N/A"} ${op.usuario_operativo?.apellido || ""}`.trim(),
                        proveedor_nombre: op.proveedor?.nombre_empresa || "N/A",
                        agente_nombre: op.agente ? `${op.agente.nombre || ""} ${op.agente.apellido || ""}`.trim() : null,
            ...
            if (mounted) {
                showNotification('error', 'Error al cargar datos del sistema');
            }
        }
    }
...
const searchableText = `${o.id_operacion} ${o.numero_referencia_proveedor || o.referencia_booking || ''} ${o.cliente_nombre} ${
o.usuario_operativo_nombre
} ${TipoServicioDisplay[o.tipo_servicio]} ${o.proveedor_nombre || ''} ${o.agente_nombre || ''}`.toLowerCase();
...
const handleFormSubmit = async (e) => {
    ...
    // Validaciones básicas
    if (!formData.id_cliente || !formData.id_usuario_operativo || !formData.id_proveedor) {
        showNotification('error', 'Por favor complete todos los campos obligatorios (Cliente, Operativo, Proveedor)');
        return;
    }
    
    if (!formData.numero_referencia_proveedor) {
        showNotification('error', 'El campo Booking/Referencia es obligatorio');
        return;
    }
    ...
    try {
        if (selectedOperacion) {
            ...
            showNotification('success', `Operación #${selectedOperacion.id_operacion} actualizada exitosamente`);
        } else {
            ...
            showNotification('success', 
                `Operación creada exitosamente (ID: ${created?.id_operacion || created?.id || "n/a"})${cotizacionInfo}`
            );
        }
...
const openFormForEdit = (operacion) => {
    ...
    setFormData({
        ...
        fecha_estimada_arribo: operacion.fecha_estimada_arribo ? operacion.fecha_estimada_arribo.substring(0, 10) : "",
        fecha_estimada_entrega: operacion.fecha_estimada_entrega ? operacion.fecha_estimada_entrega.substring(0, 10) : "",
        numero_referencia_proveedor: operacion.numero_referencia_proveedor || "",
...
const deleteOperacion = async () => {
    ...
    try {
        ...
        showNotification('success', `Operación #${selectedOperacion.id_operacion} eliminada exitosamente`);
    } catch (err) {
        console.error('Error eliminando operación:', err);
        showNotification('error', 'Error eliminando la operación: ' + (err.message || err));
    }
};
...
const loadDemoras = async () => {
    try {
        const response = await fetch("https://pwa-sistema-logistico-backend.onrender.com/demoras");
        if (response.ok) {
...
const updateDemora = async (id, demoraData) => {
    try {
        const response = await fetch(`https://pwa-sistema-logistico-backend.onrender.com/demoras/${id}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
            },
...
const loadIncidencias = async () => {
    try {
        const response = await fetch("https://pwa-sistema-logistico-backend.onrender.com/incidencias");
        if (response.ok) {
            const data = await response.json();
            setIncidencias(data);
        }
...
const updateIncidencia = async (id, incidenciaData) => {
    try {
        const response = await fetch(`https://pwa-sistema-logistico-backend.onrender.com/incidencias/${id}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (response.ok) {
            await loadIncidencias();
            setEditingIncidencia(null);
            showNotification('success', 'Incidencia actualizada exitosamente');
        } else {
...
const deleteIncidencia = async (id) => {
    try {
        ...
        if (response.ok) {
            await loadIncidencias();
            showNotification('success', 'Incidencia eliminada exitosamente');
        } else {
...
const loadTracking = async () => {
    try {
        const response = await fetch("https://pwa-sistema-logistico-backend.onrender.com/tracking");
        if (response.ok) {
            const data = await response.json();
            setTracking(data);
        }
...
const createTracking = async (trackingData) => {
    try {
        ...
        if (response.ok) {
            ...
            showNotification('error', `Error: ${errorData.message || 'Error al registrar la ubicación'}`);
        }
    } catch (error) {
        console.error('Error al crear tracking:', error);
        showNotification('error', 'Error de conexión al registrar la ubicación');
    }
};

const updateTracking = async (id, trackingData) => {
    try {
        const response = await fetch(`https://pwa-sistema-logistico-backend.onrender.com/tracking/${id}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (response.ok) {
            ...
            showNotification('error', errorData.message || 'Error al actualizar la ubicación');
        }
    } catch (error) {
...
isDetailOpen={expandedOperacionId === o.id_operacion}
demoras={demoras}
incidencias={incidencias}
tracking={tracking}
proveedores={proveedores}
